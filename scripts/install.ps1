...
<#
    ...
    使い方:
        curl -sL https://raw.githubusercontent.com/Himeyama/DESIGN/main/scripts/install.ps1 | pwsh -c -
...
elseif ([Console]::IsInputRedirected) {
    throw "対話的な選択には端末の標準入力が必要です。`curl | pwsh -c -` のように標準入力経由でスクリプトを渡す方法では選択 UI が使えません。代わりに次のいずれかを使ってください:`n  pwsh -c `"irm $RawRoot/scripts/install.ps1 | iex`"`n  curl -sL $RawRoot/scripts/install.ps1 -o install.ps1; pwsh ./install.ps1`n  pwsh ./scripts/install.ps1 -Skills <name1>,<name2>"
}
else {
